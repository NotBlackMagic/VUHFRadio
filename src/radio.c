...
//	while(AX5043SynthGetAutoRangingB(RADIO_UHF));	//Wait for Auto Ranging Complete
	...
	//Set Modulation for TX Mode
	AX5043GeneralSetModulation(RADIO_UHF, AFSK);				//AFSK
...
//	RadioUHFSetRXBitrate(1200);
	...
	AX5043RXParamSetRXDatarate(RADIO_UHF, 0x099A);		//~1200 bits/s
	AX5043RXParamSetRXMaximumDatarateOffset(RADIO_UHF, 0x00);
	AX5043RXParamSetRXMaximumFrequencyOffset(RADIO_UHF, 0xCCCC);	//Set to 10kHz (0x28F6)
...
//
//	//Set Datarates
//	uint32_t rxDr = (uint32_t)((float)(FXTAL << 7) / (vhfRadioConfiguration.datarate * decimation) + 0.5f);
//	AX5043RXParamSetRXDatarate(RADIO_VHF, rxDr);
//	uint32_t txDr = (uint32_t)((vhfRadioConfiguration.datarate * (16777216.f / FXTAL)) + 0.5f);
...
uint8_t RadioUHFModConfig(RadioConfigStruct configuration) {
	...
	if(uhfRadioConfiguration.modulation == FSK || uhfRadioConfiguration.modulation == MSK) {
		...
		//OQPSK BW = 1/2 * Bitrate
		modulationRXBW = uhfRadioConfiguration.datarate >> 1;
	}
...
//		}
	...
	AX5043PacketSetGainTimingRecovery2(RADIO_UHF, man, exp);
